...
import {
  ITicket,
  ITicketContextFetchState,
  useTicketContext,
} from "@/context.state/ticket";
...
function Page() {
  ...
  const getTickets = async ({
    ...
    fetchData({ loader, url: "/tickets/all" })
      .then(({ resData: data, resErr }) => {
        ...
        handlers.setLocalState({ key: "displayTickets", value: allTickets });
      })
      .catch((err) => {});
  };

  useEffect(() => {
    if (state.local.allTickets.length === 0) {
      getTickets({ loader: "fetchingTickets" });
    }
  }, []);
  ...
  return (
    <div className="w-full h-full flex flex-col bg-f9f7f8">
        ...
        leading={
          <div className="w-fit h-full flex gap-[36px]">
            {[
              ...
              <div
                onClick={() =>
                  handlers.setInputState({ key: "status", value: name })
                }
      ...
      {/* //!Modal */}
      <Modal
        containerClassName="w-[25vw] h-fit p-0 rounded-[20px]"
        style={{}}
      />
      {/* //!Modal */}
    </div>
