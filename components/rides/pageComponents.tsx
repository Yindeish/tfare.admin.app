...
function RideGridView() {
    ...
    return (<div className="w-[85%] h-[calc(100%-74px)] mx-auto py-[1em] bg-f9f7f8 overflow-y-scroll">
        ...
        <div className="w-full h-fit grid grid-cols-3 gap-[1em]">
            {state.local.allRides?.map((item: any, index) => (
                <div
                    ...
                    <div className="w-full h-fit flex items-start justify-between">
                        ...
                        <div className="w-[70%] flex flex-col gap-[12px]">
                            <span className="font-medium text-[14px] text-black">Ride {item?.id}</span>

                            <span className="inline-flex gap-[0.3em] items-center w-fit h-fit">
                                <LuClock3 className="text-27AE65 w-[16px] h-[16px]" />
                                <span className="font-normal text-[12px] text-747474">{item?.time}</span></span>
                        </div>

                        <Expand onClick={() => {
                            handlers.setLocalState({
                                key: 'selectedRide', value: Item
                            })
                    ...
                    <KeyValueBlock
                        keyValueArray={
                            state.inputs.method === 'order' ?
                                [
                                    { key: 'Driver Name', value: item?.driverName },
                                    { key: 'Pickup', value: item?.pickup },
                                    { key: 'Endpoint', value: item?.endpoint },
                                    { key: 'Current Passengers', value: item?.currentPassengers },
                                ]
                                :
                                [
