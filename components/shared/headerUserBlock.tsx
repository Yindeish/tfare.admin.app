...
function HeaderUserBlock() {
    const {state: {local: {user}}} = useAuthContext()

    return (
        <div className="flex items-center gap-[1em] w-fit h-fit cursor-pointer">
            <img className="w-[50px] h-[50px] rounded-full" src={user?.picture} alt="" />

            <div className="flex flex-col gap-[0.3em] ">
                <span className="font-bold text-[16px] text-black">{user?.fullName}</span>
                <span className="font-normal text-[16px] text-747474">{user?.email}</span>
            </div>
